...
# Load dataset
df = pd.read_csv('../Data-Science-Capstone-Project/data/processed/cleaned_data.csv')

# Load the pipeline
with open('../Data-Science-Capstone-Project/models/Random Forest.pkl', 'rb') as file:
    pipeline = pickle.load(file)

def filter_cars_by_company(selected_company, df):
    sorted_df = df.sort_values(['Brand', 'Model'], ascending=True)
    filtered_cars = sorted_df[sorted_df['Brand'] == selected_company]['Model'].unique()
